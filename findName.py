...
embeddings_index = {}
with open(os.path.join(GLOVE_DIR, 'glove.6B.100d.txt'), encoding="ISO-8859-1") as f:
    for line in f:
...
model_2 = Sequential([Embedding(vocab_size+1, 50, input_length=MAX_SEQUENCE_LENGTH),
...
for vec in mo:
    ...
    print(vec)
    i += 1
    print(" ".join(map(lambda x: reverse_word_map[x], vec)))
    print()
